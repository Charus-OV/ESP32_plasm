...
void WebInterface::processWebSocketCommand(String message) {
  ...
  } else if (command == "set_coordinate") {
    String axis = doc["axis"];
    float value = doc["value"];
    Serial.printf("🎯 Установка координаты %s: %.3f\n", axis.c_str(), value);
  } else if (command == "set_all_coordinates") {
    float x = doc["x"];
    float y = doc["y"];
    float z = doc["z"];
    Serial.printf("🎯 Установка всех координат: X=%.3f, Y=%.3f, Z=%.3f\n", x, y, z);
